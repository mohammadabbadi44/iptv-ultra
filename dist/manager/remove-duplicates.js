...
function parseChannels(file) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].startsWith('#EXTINF')) {
            ...
            const url = lines[i + 1] || '';
            const name = ((_a = extinf.split(',').pop()) === null || _a === void 0 ? void 0 : _a.trim()) || '';
...
function removeDuplicatesFromFile(file) {
    ...
    fs_1.default.writeFileSync(file, lines.filter(Boolean).join('\n'), 'utf8');
